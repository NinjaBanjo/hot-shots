...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  // bind the _reconnectHandler so we can removeListener by reference
  ...
  this.udsGracefulErrorHandling = options.udsGracefulErrorHandling || true;
  this.udsGracefulRestartRateLimit = options.udsGracefulRestartRateLimit || 1000;
...
Client.prototype.sendMessage = function (message, callback) {
  ...
  const handleCallback = (err) => {
    ...
    if (callback) {
      ...
      if (this.udsGracefulErrorHandling) {
        this._reconnectHandler(err);
      }
    }
  };
...
Client.prototype._reconnectHandler = function (err) {
  console.log('fancy error meeee', err);
  const code = err.code;
  switch (code) {
    case 107:
    case 111: {
      if (Date.now() - this.lastSocketCreateTime >= this.socketCreateLimit) {
        console.log('[hot-shots] attempting to re-create the socket');
        // recreate the socket, but only once per 30 seconds
        if (this.errorHandler) {
          this.socket.removeListener('error', this.errorHandler);
        }

        this.socket.removeListener('error', this._reconnectHandler);
        ...
        this.lastSocketCreateTime = Date.now();
        console.log('[hot-shots] socket re-created successfully');
      }
      break;
    }
    default: {
...
Client.prototype._bindReconnectHandler = function() {
  this.socket.on('error', this._reconnectHandler);
};
